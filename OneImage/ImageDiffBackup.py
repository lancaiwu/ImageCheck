...
def insertRecord(db, uid, phoneNumber, imagePath1, imagePath2, checkResultType, checkResult, time):
    ...
    if imagePath2 is not None:
        ...
        imagePath2 = ""
    sql = "INSERT INTO t_yz_record(phoneNumber, uid, imagePath1, imagePath2, checkResultType, checkResult, time) VALUE (%s,%d,%s,%s,%d,%d,%d)" % (
        phoneNumber, uid, r"'" + imagePath1 + "'", r"'" + imagePath2 + "'", checkResultType, checkResult, time)
    cursor = db.cursor()
    cursor.execute(sql)
    db.commit()
    return
...
#  验证图
agr2 = "C:\\Users\\Administrator.SKY-20170712YLK\\Desktop\\screen\\13.jpg"
